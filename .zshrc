...
function () {
  ...
  export PS1="%F{green}${SSH_TTY:+%n@%m}%f%B${SSH_TTY:+:}%b%F{blue}%B%(5~|…/%3~|%~)%b%F{yellow}%B%(1j.*.)%(?..!)%b%f %B${SUFFIX}%b "
  if [[ -n "$TMUXING" ]]; then
...
ZLOCAL_RC=$HOME/.zshrc.local
test -f $ZLOCAL_RC && source $ZLOCAL_RC

ZHOST_RC=$HOME/.zsh/host/$(hostname -s | tr '[:upper:]' '[:lower:]')
test -f $ZHOST_RC && source $ZHOST_RC

ALIAS_RC=$HOME/.aliases
test -f $ALIAS_RC && source $ALIAS_RC

HOST_RC=$HOME/.host
test -f $HOST_RC && source $HOST_RC

# Update prompt with custom hostname
if [[ "$HN" != "" ]]; then
  hn="$HN"
else
  hn="%m"
fi
RPROMPT_BASE="\${vcs_info_msg_0_}%F{blue}$hn%f"
export RPROMPT=RPROMPT_BASE
