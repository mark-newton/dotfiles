# The prompt
PROMPT='%{$fg_bold[black]%}%D{%H:%M:%S}%{$reset_color%} [$(_user_host)$(_python_venv) %{$fg[cyan]%}%c%{$reset_color%} $(git_prompt_info)%{$reset_color%}] ${ret_status}%{$reset_color%}'
...
function _user_host() {
  ...
  echo "$me%{$reset_color%}@%{$fg_bold[white]%}$hn%{$reset_color%}"
}

# Determine if there is an active Python virtual environment
function _python_venv() {
  if [[ $VIRTUAL_ENV != "" ]]; then
    echo "%{$fg[cyan]%}(${VIRTUAL_ENV##*/})%{$reset_color%} "
  fi
}
